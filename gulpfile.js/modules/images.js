...
function createImage(env) {
	return function images(cb2) {
		let total = 0;
		let loaded = 0;
		function isEnd() {
			loaded++;
		...
		glob(`${config.src}**/*.{png,jpg,gif,ico,svg}`, (error, files) => {
			...
			files.map((file, key) => {
				...
				var compressed = db.get("images").find({ file: relativePath }).value();
				if (!stat || !stat.size || !compressed || compressed.size != stat.size) {
					tinypng
						...
						.then((res) => {
							if (!!res) {
								...
								if (!compressed) {
									db.get("images")
										.push({ file: relativePath, size: stat.size || "" })
										.write();
								} else {
									db.get("images")
										.find({ file: relativePath })
										.assign({ size: stat.size || "" })
										.write();
								}
							}
